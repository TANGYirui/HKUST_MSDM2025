...
def neighbors_positions(board, padding=2):
    ...
    If board empty, return center-top preference.
    """
    occ = []
    for u, v in LEGAL_POSITIONS:
        if board[u][v] != 0:
            occ.append((u, v))
    if not occ:
        # choose a sensible start near top-middle
        mid = (0, (BLINE - 1) // 2)
        if mid in LEGAL_SET:
            return [mid]
...
def rollout_policy(board, turn):
    ...
    for (u, v) in cands:
        ...
        cu, cv = 7, 4  # center of triangular board is NOT (7,7)
