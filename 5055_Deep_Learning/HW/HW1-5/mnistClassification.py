...
class Linear(Node):
    ...
    def __init__(self, input_shape, output_shape, weight=None, bias=None):
        ...
        if weight is None:
            ...
            '''
            fan_in = input_shape
            fan_out = output_shape
            limit = np.sqrt(6 / (fan_in + fan_out))
            weight = np.random.uniform(-limit, limit, (input_shape, output_shape))
...
class CrossEntropy(Node):
    ...
    def forward(self, x, l):
        ...
        loss = -np.sum(l * np.log(x + 1e-12)) / batch_size
        return loss
    ...
    def backward(self, delta):
        ...
        delta = (self.x - self.l) / batch_size
        return delta
...
def load_MNIST():
    ...
    '''
    #base = "http://yann.lecun.com/exdb/mnist/" # original site is down
    base = "https://web.archive.org/web/20220331130319/https://yann.lecun.com/exdb/mnist/" # use the archived version
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epoch):
        for j in range(num_iteration):
            ...
            for term in net:
                if term.name == 'linear':
                    term.parameters[0] -= learning_rate * term.parameters_deltas[0]
                    term.parameters[1] -= learning_rate * term.parameters_deltas[1]
    ...
    layer_string = []
    layer_paramters = []
    npy_path = os.path.join(current_dir, 'npMnistParameters.npy')
    ...
    json_path = os.path.join(current_dir, 'npMnistStructure.json')
